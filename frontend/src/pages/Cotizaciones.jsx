...
function ModalPreview({ cotizacion, onCerrar, onDescargar, onWhatsApp, onEmail }) {
  ...
  return (
    <div style={{
      ...
      padding: mob ? '0' : '24px 16px'
    }} onClick={e => { if (e.target === e.currentTarget) onCerrar(); }}>
      <div style={{
        background: 'white',
        borderRadius: mob ? '0' : '16px',
        width: '100%', maxWidth: mob ? '100%' : '760px',
        minHeight: mob ? '100vh' : 'auto',
        overflow: 'hidden', boxShadow: '0 20px 60px rgba(0,0,0,0.3)'
      }}>

        {/* Barra superior */}
        <div style={{ background: '#4F6EF7', padding: mob ? '12px 14px' : '14px 24px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
            <span style={{ color: 'white', fontWeight: 700, fontSize: mob ? '13px' : '15px' }}>
              Propuesta #{String(cotizacion.numero).padStart(4, '0')}
            </span>
            <button onClick={onCerrar}
              style={{ background: 'rgba(255,255,255,0.2)', color: 'white', border: 'none', borderRadius: '8px', width: '32px', height: '32px', cursor: 'pointer', fontSize: '18px', lineHeight: 1, flexShrink: 0 }}>
              ×
            </button>
          </div>
        ...
        <div style={{ padding: mob ? '16px' : '32px 40px', fontFamily: 'Arial, sans-serif' }}>

          {/* Encabezado: Logo + Datos empresa */}
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px', flexDirection: mob ? 'column' : 'row', gap: mob ? '10px' : '0' }}>
            <img src="/logo.png" alt="GPS Tracker Panamá" style={{ height: mob ? '48px' : '64px', maxWidth: '150px', objectFit: 'contain' }} />
            <div style={{ textAlign: mob ? 'left' : 'right', fontSize: '11px', color: '#555', lineHeight: 1.7 }}>
              <div style={{ fontSize: '14px', fontWeight: 700, color: '#1a1a2e', marginBottom: '2px' }}>GPS Tracker Panamá</div>
              <div>Zona Industrial, Costa del Este, Panamá</div>
          ...
          {/* Línea azul divisora */}
          <div style={{ height: '2px', background: '#4F6EF7', marginBottom: '16px', borderRadius: '1px' }}></div>

          {/* Título + número */}
          <div style={{ background: '#f0f4ff', borderRadius: '8px', padding: '10px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
            <div style={{ color: '#4F6EF7', fontWeight: 800, fontSize: mob ? '15px' : '18px' }}>PROPUESTA</div>
          ...
          <div style={{ borderTop: '1px solid #e5e7eb', paddingTop: '14px', marginBottom: '16px' }}>
            <div style={{ fontSize: '10px', fontWeight: 700, color: '#6b7280', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '4px' }}>Cliente:</div>
            <div style={{ fontSize: mob ? '14px' : '16px', fontWeight: 700, color: '#1a1a2e' }}>{cotizacion.nombre_cliente}</div>
          ...
          <div style={{ overflowX: 'auto', marginBottom: '16px', borderRadius: '8px' }}>
            <table style={{ width: '100%', minWidth: '460px', borderCollapse: 'collapse', fontSize: mob ? '12px' : '13px' }}>
              <thead>
                <tr style={{ background: '#4F6EF7' }}>
                  <th style={{ padding: '9px 12px', textAlign: 'left', color: 'white', fontWeight: 700 }}>PRODUCTO / SERVICIO GPS</th>
                  <th style={{ padding: '9px 10px', textAlign: 'right', color: 'white', fontWeight: 700, whiteSpace: 'nowrap', width: '70px' }}>PRECIO</th>
                  <th style={{ padding: '9px 10px', textAlign: 'center', color: 'white', fontWeight: 700, width: '50px' }}>CANT.</th>
                  <th style={{ padding: '9px 10px', textAlign: 'right', color: 'white', fontWeight: 700, width: '70px' }}>DESC.</th>
                  <th style={{ padding: '9px 10px', textAlign: 'right', color: 'white', fontWeight: 700, width: '75px' }}>TOTAL</th>
                </tr>
              </thead>
              <tbody>
                {items.length === 0 ? (
                  ...
                  return (
                    <tr key={i} style={{ background: i % 2 === 0 ? '#f8faff' : 'white', borderBottom: '1px solid #e5e7eb' }}>
                      <td style={{ padding: '9px 12px' }}>
                        ...
                        {item.descripcion && <div style={{ fontSize: '11px', color: '#6b7280', marginTop: '2px', lineHeight: 1.4 }}>{item.descripcion}</div>}
                      </td>
                      <td style={{ padding: '9px 10px', textAlign: 'right', color: '#374151', whiteSpace: 'nowrap' }}>B/.{precio.toFixed(2)}</td>
                      <td style={{ padding: '9px 10px', textAlign: 'center', color: '#374151' }}>{cantidad}</td>
                      <td style={{ padding: '9px 10px', textAlign: 'right', color: '#374151', whiteSpace: 'nowrap' }}>{desc > 0 ? `B/.${desc.toFixed(2)}` : '—'}</td>
                      <td style={{ padding: '9px 10px', textAlign: 'right', fontWeight: 700, color: '#111827', whiteSpace: 'nowrap' }}>B/.{importe.toFixed(2)}</td>
                    </tr>
                  );
                })}
              </tbody>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '16px' }}>
            <div style={{ minWidth: mob ? '100%' : '240px' }}>
              ...
              {descuento > 0 && (
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '4px 0', fontSize: '13px', color: '#555' }}>
                  <span>Descuento:</span><span>- B/. {descuento.toFixed(2)}</span>
                </div>
              )}
          ...
          {/* Notas */}
          {cotizacion.notas && (
            <div style={{ background: '#f9fafb', borderRadius: '8px', padding: '10px 14px', marginBottom: '14px', fontSize: '12px' }}>
          ...
          {/* Términos */}
          <div style={{ fontSize: '11px', color: '#9ca3af', borderTop: '1px solid #e5e7eb', paddingTop: '10px', marginBottom: '14px' }}>
            Esta propuesta es válida por 15 días a partir de su emisión. Precios en Balboas panameños (B/.) equivalentes a USD.
          </div>

          {/* Footer */}
          <div style={{ background: '#f0f4ff', borderRadius: '8px', padding: '10px 14px', display: 'flex', justifyContent: 'space-between', fontSize: '11px', borderTop: '2px solid #4F6EF7', flexWrap: 'wrap', gap: '6px' }}>
            <div>
